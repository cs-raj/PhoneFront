...
describe('/api/header', () => {
  ...
  it('should fetch header data successfully', async () => {
    // Arrange
    const request = createMockNextRequest('http://localhost:3000/api/header');
    mockGetAllEntries.mockResolvedValue(mockContentstackDeliveryResponses.header.success);
    ...
    // Assert
    expect(mockGetAllEntries).toHaveBeenCalledWith('header', undefined);
    expect(response.status).toBe(200);
    expect(responseData).toEqual({
      ...mockContentstackDeliveryResponses.header.success.entries[0],
      personalized: false,
      variantParam: null
    });
  });
  ...
  it('should handle personalized requests', async () => {
    ...
    // Assert
    expect(mockGetAllEntries).toHaveBeenCalledWith('header', 'exp_123_var_456');
    expect(response.status).toBe(200);
  ...
  it('should handle empty Contentstack response', async () => {
    // Arrange
    const request = createMockNextRequest('http://localhost:3000/api/header');
    mockGetAllEntries.mockResolvedValue(mockContentstackDeliveryResponses.header.empty);
  ...
  it('should handle Contentstack API errors', async () => {
    // Arrange
    const request = createMockNextRequest('http://localhost:3000/api/header');
    mockGetAllEntries.mockRejectedValue(mockContentstackDeliveryResponses.header.error);
